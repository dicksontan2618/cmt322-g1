        ...
        <div className="px-8 mb-8 lg:px-16 lg:py-8">
            ...
            <div className="grid grid-cols-1 gap-y-6 md:grid-cols-2 md:gap-x-4 lg:grid-cols-3 lg:gap-x-4">
                {workshops && workshops.map((workshop: { id: string; thumbnail_img: string; name: string; tag: string; date: string; venue: string; }, index: Key | null | undefined) => (
                    <WorkshopCard key={index} slug={workshop.id} imageSrc={workshop.thumbnail_img} title={workshop.name} category={workshop.tag} date={workshop.date} venue={workshop.venue} colorCode="#ED4989" canEdit={false} />
                ))}
            </div>
        </div>
    )
